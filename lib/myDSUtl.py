...
def myDSUtlFlagVerbosity(ix):
    if                   (ix % 1000 == 0)  or \
        ((ix < 1000) and (ix % 100  == 0)) or \
        ((ix < 100 ) and (ix % 10   == 0)) or \
        (ix < 10  ):
        return True
...
def myDSUtlDisplayDf(df, label = None, indent = 2, nRows = 10):
    ...
    if (label is None) and (hasattr(df, "myLabel")):
        label = df.myLabel
    print("\n%smyDSUtlDisplayDf: DataFrame: %s| shape: %s" % (' ' * indent, label, str(df.shape)))
    
    indexRange = df.index.sort_values()
    indexRange = (indexRange[0], indexRange[-1])
    print("%s  Index: type:%s| range:%s" % (' ' * indent, type(df.index), str(indexRange)))
    
    print(df.describe(include = 'all'))
    print("%s  sample records:" % (' ' * indent))
    print(df.iloc[:nRows, :10])
    if (df.shape[0] > nRows):
        print(df.iloc[-nRows:, -10:])
